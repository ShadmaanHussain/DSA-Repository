...
BinaryTreeNode<int>* BSTfromArray(vector<int> arr, int start, int end) {
    if(start > end) return NULL;
    int mid = (start + end) / 2;
    BinaryTreeNode<int>* root = new BinaryTreeNode<int>(arr[mid]);
    BinaryTreeNode<int>* left = BSTfromArray(arr, start, mid - 1);
    BinaryTreeNode<int>* right = BSTfromArray(arr, mid + 1, end);
    root->left = left;
    root->right = right;
    return root;
}
...
int main()
{
    ...
    vector<int> vec {1, 2, 3, 4, 5, 6, 7, 11};
    BinaryTreeNode<int> *root = BSTfromArray(vec, 0, vec.size() - 1);
