...
vector<int>* PathFromRoot(BinaryTreeNode<int>* root, int node) {
    if(root == NULL) return NULL;
    if(root->data == node) {
        vector<int>* ans = new vector<int>{root->data};
        return ans;
    }
    vector<int>* left = PathFromRoot(root->left, node);
    if(left != NULL) {
        left->push_back(root->data);
        return left;
    }
    vector<int>* right = PathFromRoot(root->right, node);
    if(right != NULL) {
...
int main()
{
    ...
    vector<int> *path = PathFromRoot(root, 8);
    for(int i : *path) {
        cout << i << " ";
    }
    delete root;
